...
// 1. Photo Live Preview + Instant Local Compressor (100% Real User Photo)
function previewPhoto(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            const img = new Image();
            img.onload = function() {
                // Browser canvas se photo ko 30KB ka compress karo
                const canvas = document.createElement('canvas');
                const ctx = canvas.getContext('2d');
                
                // Max width 400px (Lightweight & Super Fast)
                const maxWidth = 400;
                ...
                // REAL USER PHOTO SAVE KARO (NO UNSPLASH)
                capturedImage = compressedPhoto;
                console.log("✅ User's Real Photo Captured!");
            };
